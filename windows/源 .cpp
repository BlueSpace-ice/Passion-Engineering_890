...
//筛选出四个直角的灯条，返回下标
vector<int> handleLight(vector<vector<cv::Point>> contours, vector<RotatedRect> minRect, vector<Triangle> triangle, Mat cap)
{
	...
	if (Minindex.size() == 3) {
		...
		vector<Point2f> p3_to_circles = detectmask(minRect, Minindex,cap);//返回三个点待选点的坐标(不一定是三个）
...
{
	// 导入图像
	Mat image = imread("D:\\桌面\\兑换站视频\\兑换站\\兑换站\\output_folder\\image_1132.png");
	if (!image.empty())
	{
		capcols = image.cols;
		caprows = image.rows;
